...
import '../../../../../contracts/PhoswapV3Factory.sol';
...
contract SetupPhoswap {
    PhoswapV3Pool public pool;
    TestERC20 token0;
    TestERC20 token1;
    ...
    PhoswapV3Factory factory;

    constructor(TestERC20 _token0, TestERC20 _token1) public {
        factory = new PhoswapV3Factory();
        token0 = _token0;
        token1 = _token1;
    }

    function createPool(uint24 _fee, uint160 _startPrice) public {
        pool = PhoswapV3Pool(factory.createPool(address(token0), address(token1), _fee));
        pool.initialize(_startPrice);
    }
}

contract PhoswapMinter {
    PhoswapV3Pool pool;
    TestERC20 token0;
    TestERC20 token1;
    ...
    function setPool(PhoswapV3Pool _pool) public {
        pool = _pool;
    }
